...
func PrintVersion(cfg *config.Config) *cli.Command {
	return &cli.Command{
		...
		Flags: flagset.ListSearchWithConfig(cfg),
		Action: func(c *cli.Context) error {
			reg := registry.GetRegistry()
			services, err := reg.GetService(cfg.Service.Namespace + "." + cfg.Service.Name)
			if err != nil {
				fmt.Println(fmt.Errorf("could not get search services from the registry: %v", err))
				return err
			}

			if len(services) == 0 {
				fmt.Println("No running search service found.")
				return nil
			}
